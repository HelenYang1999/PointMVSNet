...
class PointMVSNet(nn.Module):
    ...
    def forward(self, data_batch, img_scales, inter_scales, isFlow, isTest=False):
        ...
        img_list = data_batch["img_list"] #[B,3,3,512,640] B为Batch3个通道3个视角3个通道512高640宽
        cam_params_list = data_batch["cam_params_list"] #[B,3,2,4,4] B为Batch3个视角，0维是外参，1维是内参

        cam_extrinsic = cam_params_list[:, :, 0, :3, :4].clone()  # 外参矩阵(B, V, 3, 4) V=3
        R = cam_extrinsic[:, :, :3, :3] # (B, V, 3, 3) 旋转矩阵
        t = cam_extrinsic[:, :, :3, 3].unsqueeze(-1) # (B, V, 3, 1) 平移矩阵
        R_inv = torch.inverse(R) # 旋转矩阵的逆，仍然为 (B, V, 3, 3)
        cam_intrinsic = cam_params_list[:, :, 1, :3, :3].clone() # 内参矩阵(B,V,3,3)
        cam_intrinsic[:, :, :2, :3] = cam_intrinsic[:, :, :2, :3] / 2.0 #todo 为什么要除以2
        if isTest:
            cam_intrinsic[:, :, :2, :3] = cam_intrinsic[:, :, :2, :3] / 4.0

        depth_start = cam_params_list[:, 0, 1, 3, 0] # 最小深度
        depth_interval = cam_params_list[:, 0, 1, 3, 1] # 深度间隔
        num_depth = cam_params_list[0, 0, 1, 3, 2].long() # 深度数目

        depth_end = depth_start + (num_depth - 1) * depth_interval # 最大深度

        batch_size, num_view, img_channel, img_height, img_width = list(img_list.size())

        coarse_feature_maps = []
        for i in range(num_view):
            curr_img = img_list[:, i, :, :, :] # [B,3,512,640]
            curr_feature_map = self.coarse_img_conv(curr_img)["conv3"] # [B,64,64,80] 每张图像的特征
            coarse_feature_maps.append(curr_feature_map)

        feature_list = torch.stack(coarse_feature_maps, dim=1) # [B,V,64,64,80]
        ...
        feature_map_indices_grid = get_pixel_grids(feature_height, feature_width) \
            .view(1, 1, 3, -1).expand(batch_size, 1, 3, -1).to(img_list.device) #[4,1,3,5120]
